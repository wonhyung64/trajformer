...
scaled_y = 223 - np.round((traj["y"] - traj["y"].min())/(traj["y"].max()-traj["y"].min()) * 223).astype(int)
scaled_x = np.round((traj["x"] - traj["x"].min())/(traj["x"].max()-traj["x"].min()) * 223).astype(int)
index = torch.LongTensor([np.vstack([scaled_x, scaled_y]).T])
out = torch.zeros(index.size(0), 224, 224)
out[torch.arange(out.size(0)).unsqueeze(1), index[:, :, 1], index[:, :, 0]] = 1.
